...
<?php
	...
	if( ! isset( $_GET['authid'] ) || $_GET['authid'] == null ) {
		header( "Location: 404.php" );
		exit();
	// if( ! isset( $_GET['authid'] ) || $_GET['authid'] == null ) End here...
	} else {
		if( preg_match( '/[^0-9]/', $_GET['authid'] ) ) {
			header( "Location: 404.php" );
			exit();
		} else {
			$authid = (int) $_GET['authid'];
			$authid = mysqli_real_escape_string( $db->link, Format::validation( $authid ) );
		}
	}
...
<?php
	...
	$query = "SELECT * FROM {$post_table} WHERE userid = '{$authid}'";
